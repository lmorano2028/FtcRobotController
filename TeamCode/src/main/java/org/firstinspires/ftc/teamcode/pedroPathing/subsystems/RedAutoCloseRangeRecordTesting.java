...
@Autonomous
public class RedAutoCloseRangeRecordTesting extends OpMode {
    ...
    // ==========================================================
    private static final double RED_GOAL_PX = 144; //128.5; // prevouslt 127.6425
    private static final double RED_GOAL_PY = 130; //122.5;//previously 130.3727
    ...
    // Note: these Poses are not used directly by the path builder (you hard-code in buildPaths),
    // but keeping them here as you had them.
    ...
    private final Pose openGatePos = new Pose(119, 74, Math.toRadians(-90));

    private final Pose thirdReloadStart3 = new Pose(95.000, 41.0, Math.toRadians(0));
    private final Pose thirdReloadEnd3   = new Pose(123.000, 41.0, Math.toRadians(0));

    // IMPORTANT: final shot is now at shotpos (per your new paths), so we use this as the final reference.
    ...
    public void buildPaths() {
        ...
        // ===== Reload 2 (unchanged up to sweep end at 24,63) =====
        ...
        // ===== NEW: After reload2 end -> open gate -> shotpos =====
        // 2ndofreload2toopengat
        ...
        switch(pathState) {
            ...
            case RELOAD2_DECOMPRESS:
                ...
                if (pathTimer.getElapsedTimeSeconds() >= RELOAD_DECOMPRESS_SEC) {
                    ...
                    // ===== CHANGED: go hit the gate route instead of driving straight back =====
            ...
            case DRIVE_OPEN_GATE_TO_SHOTPOS:
                ...
                if (follower.isBusy()) {
                    ...
                    // Now we are back at shotpos and continue the same volley-3 logic
            ...
            case RELOAD3_DECOMPRESS:
                ...
                if (pathTimer.getElapsedTimeSeconds() >= RELOAD_DECOMPRESS_SEC) {
                    ...
                    // ===== CHANGED: Path10 returns to shotpos (instead of open-gate -> finalShot) =====
