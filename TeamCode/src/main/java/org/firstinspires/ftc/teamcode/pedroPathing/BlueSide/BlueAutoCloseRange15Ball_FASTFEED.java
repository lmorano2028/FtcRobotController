...
@Autonomous(name = "BlueAutoCloseRange15Ball", group = "Blue")
public class BlueAutoCloseRange15Ball_FASTFEED extends OpMode {
    ...
    // ==========================================================
    // ODOMETRY + VISION TRIM TURRET AIM (FIELD GOAL CONSTANTS)
    // ==========================================================
    // CURRENTLY THIS WAS "RED ONLY" IN YOUR RED FILE.
    // For BLUE SIDE you almost certainly want:
    //   BLUE_GOAL_PX = 2   (mirror of 142 if field width is 144)
    //   BLUE_GOAL_PY = 142
    // and the BLUE speaker tag id (depends on your game config).
    //
    // If your old RED tag was 24, BLUE is commonly the other one.
    // VERIFY in your AprilTag list / Limelight pipeline.
    private static final double GOAL_PX = 2;     // <-- set for BLUE
    private static final double GOAL_PY = 140;   // <-- set for BLUE
    private static final int GOAL_TAG_ID = 20;   // <-- VERIFY (was 24 in red)
    ...
    private static final double DRIVE_INTAKE2_PWR =  0.25;
    ...
    // ==========================================================
    // Gate reload wait (1.00 sec)
    // ==========================================================
    private static final double GATE_RELOAD_WAIT_SEC = 1.5;
    private boolean gateReloadWaitStarted = false;
    ...
    // ==========================================================
    // POSES (REPLACED WITH YOUR BLUE-SIDE POINTS)
    // ==========================================================
    ...
    private final Pose gateReloadPose      = new Pose(16.000, 53.000, Math.toRadians(135)); // using your secondReloadEnd as "reload"
    ...
    public void buildPaths() {
        ...
        // Gate move using your blue points:
        // shotPose -> openGatePos -> secondReloadEnd
    ...
    public void statePathUpdate() {
        switch (pathState) {
            ...
            case DRIVE_SHOT_TO_GATERELOAD_START:
                driveCollect(shotToGateReloadStart, gateReloadStartPose, PathState.DRIVE_GATERELOAD_START_TO_RELOAD);
                break;
            ...
            case WAIT_GATERELOAD_1P5S:
                ...
                if (!gateReloadWaitStarted) {
                    gateReloadWaitStarted = true;
            ...
            case DRIVE_GATERELOAD_TO_SHOT:
                driveCollect(gateReloadToShot, shotPose, PathState.SHOOT_4);
                break;
            ...
            case DRIVE_RELOAD3_END_TO_FINALSHOT:
                driveCollect(reload3EndToFinalShot, finalShotPose, PathState.SHOOT_FINAL);
                break;
    ...
    public void init() {
        ...
        shooterSys = new ShooterSubsystem(
                ...
                new double[]{1020, 1045, 1110, 1180, 1260, 1440},
                new double[]{1040, 1060, 1120, 1200, 1280, 1470},
                new double[]{1050, 1070, 1130, 1210, 1300, 1500},
                110
        );
    ...
    public void loop() {
        ...
        telemetry.addData("gateReloadWaitStarted", gateReloadWaitStarted);
    ...
    private double getAutoAimShooterDistanceIn() {
        ...
        return Math.hypot(GOAL_PX - turretX, GOAL_PY - turretY);
    }
    ...
    private double getAutoAimTurretCmd() {
        ...
        double bearingDegField = Math.toDegrees(Math.atan2(GOAL_PY - turretY, GOAL_PX - turretX));
    ...
    private void updateVisionTrim() {
        ...
        for (LLResultTypes.FiducialResult t : tags) {
            ...
            if (id == GOAL_TAG_ID) { chosen = t; break; }
        }
